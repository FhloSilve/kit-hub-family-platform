...
type ActionType = "task.create" | "task.complete" | "event.create" | "meal.plan" | "routine.create" | "routine.complete";
type ProposedAction = { type: ActionType; summary: string; payload: Record<string, unknown> };
type Access = { user: { id: string; name?: string | null }; householdId: string; role: string; response?: never } | { response: ReturnType<typeof apiError> };
...
async function access(c: Ctx): Promise<Access> {
  ...
  return { user: session.user, householdId, role: membership.role };
}
...
async function context(c: Ctx, householdId: string, userId: string, role: string) {
  const now = isoNow();
  ...
  const [household, locale, members, tasks, events, meals, routines, activity, taskManage, calendarManage, mealsManage] = await Promise.all([
    ...
    c.env.DB.prepare(`SELECT id,title,description,location,starts_at startsAt,ends_at endsAt,all_day allDay,event_type eventType FROM everyday_events WHERE household_id=? AND starts_at>=? AND starts_at<=? ORDER BY starts_at LIMIT 30`).bind(householdId, now, weekEnd).all(),
    ...
    hasPermission(c, householdId, userId, "tasks.manage"),
    hasPermission(c, householdId, userId, "calendar.manage"),
    hasPermission(c, householdId, userId, "meals.manage"),
  ]);
  ...
  return {
    ...
    capabilities: { tasks: taskManage, calendar: calendarManage, meals: mealsManage, routines: routineManager(role) },
...
function parseEnvelope(raw: string): { answer: string; action?: { type?: unknown; payload?: unknown } | null } {
  ...
  if (first >= 0 && last > first) {
    try {
      const parsed = JSON.parse(stripped.slice(first, last + 1)) as any;
      return { answer: clean(parsed?.answer, 4000) || "I can help with that.", action: parsed?.action ?? null };
    } catch { /* fall through */ }
  }
  return { answer: clean(raw, 4000) || "Silvi could not form an answer yet.", action: null };
}

async function normalizeAction(c: Ctx, a: Exclude<Access, { response: ReturnType<typeof apiError> }>, raw: { type?: unknown; payload?: unknown } | null | undefined): Promise<ProposedAction | null> {
  ...
  if (!["task.create", "task.complete", "event.create", "meal.plan", "routine.create", "routine.complete"].includes(type)) return null;
  ...
  if (type === "task.create") {
    ...
    const normalized = { title, notes: optionalText(payload.notes, 2000), priority, dueAt, assigneeUserId };
    return { type, payload: normalized, summary: `Create task “${title}”${assignee ? ` for ${assignee.name}` : ""}${dueAt ? ` due ${new Date(dueAt).toLocaleString()}` : ""}.` };
  }
  ...
  if (type === "event.create") {
    ...
    const normalized = { title, description: optionalText(payload.description, 2000), location: optionalText(payload.location, 180), startsAt, endsAt, allDay, eventType, recurrence, reminderMinutes };
    return { type, payload: normalized, summary: `Add “${title}” to Calendar on ${new Date(startsAt).toLocaleString()}.` };
  }
  if (type === "meal.plan") {
    if (!(await hasPermission(c, a.householdId, a.user.id, "meals.manage"))) return null;
    const mealDate = clean(payload.mealDate, 10), title = clean(payload.title, 160);
    ...
    const cookUserId = clean(payload.cookUserId, 120) || null;
    const cook = cookUserId ? await member(c, a.householdId, cookUserId) : null; if (cookUserId && !cook) return null;
    const reminderMinutes = typeof payload.reminderMinutes === "number" ? Math.max(0, Math.min(10080, payload.reminderMinutes)) : null;
    const normalized = { mealDate, mealType, title, cookUserId, notes: optionalText(payload.notes, 1200), reminderMinutes };
    return { type, payload: normalized, summary: `Plan ${title} for ${mealType} on ${mealDate}${cook ? ` with ${cook.name} cooking` : ""}.` };
  }
  if (type === "routine.create") {
    ...
    const reminderMinutes = typeof payload.reminderMinutes === "number" ? Math.max(0, Math.min(10080, payload.reminderMinutes)) : null;
    const normalized = { title, notes: optionalText(payload.notes, 1200), cadence, assigneeUserId, nextDueAt, reminderMinutes };
    return { type, payload: normalized, summary: `Create ${cadence} routine “${title}”${assignee ? ` for ${assignee.name}` : ""}.` };
  }
...
async function executeAction(c: Ctx, a: Exclude<Access, { response: ReturnType<typeof apiError> }>, action: ProposedAction) {
  const p = action.payload as any;
  const actor = a.user.name ?? "A household member";
  if (action.type === "task.create") {
    const id = crypto.randomUUID();
    await c.env.DB.prepare("INSERT INTO everyday_tasks (id,household_id,title,notes,priority,due_at,assignee_user_id,created_by) VALUES (?,?,?,?,?,?,?,?)")
      .bind(id, a.householdId, p.title, p.notes, p.priority, p.dueAt, p.assigneeUserId, a.user.id).run();
    await recordHouseholdActivity(c, a.householdId, a.user.id, "task.add", `${actor} asked Silvi to add task “${p.title}”.`);
    return `Created task “${p.title}”.`;
  }
  if (action.type === "task.complete") {
    const task = await c.env.DB.prepare("SELECT title FROM everyday_tasks WHERE id=? AND household_id=? AND status='todo'").bind(p.taskId, a.householdId).first<{ title: string }>();
    if (!task) throw new Error("That task is no longer open.");
    await c.env.DB.prepare("UPDATE everyday_tasks SET status='done',updated_at=datetime('now') WHERE id=? AND household_id=? AND status='todo'").bind(p.taskId, a.householdId).run();
    await recordHouseholdActivity(c, a.householdId, a.user.id, "task.completed", `${actor} completed “${task.title}” with Silvi.`);
    return `Marked “${task.title}” as done.`;
  }
  if (action.type === "event.create") {
    const id = crypto.randomUUID();
    await c.env.DB.prepare(`INSERT INTO everyday_events (id,household_id,title,description,location,starts_at,ends_at,all_day,event_type,recurrence,reminder_minutes,created_by) VALUES (?,?,?,?,?,?,?,?,?,?,?,?)`)
      .bind(id, a.householdId, p.title, p.description, p.location, p.startsAt, p.endsAt, p.allDay ? 1 : 0, p.eventType, p.recurrence, p.reminderMinutes, a.user.id).run();
    await recordHouseholdActivity(c, a.householdId, a.user.id, "calendar.add", `${actor} asked Silvi to add “${p.title}” to Calendar.`);
    return `Added “${p.title}” to Calendar.`;
  }
  if (action.type === "meal.plan") {
    ...
    await c.env.DB.prepare(`INSERT INTO meal_plans (id,household_id,meal_date,meal_type,title,recipe_id,cook_user_id,notes,reminder_minutes,created_by) VALUES (?,?,?,?,?,NULL,?,?,?,?) ON CONFLICT(household_id,meal_date,meal_type) DO UPDATE SET title=excluded.title,recipe_id=NULL,cook_user_id=excluded.cook_user_id,notes=excluded.notes,reminder_minutes=excluded.reminder_minutes,updated_at=datetime('now')`)
      .bind(id, a.householdId, p.mealDate, p.mealType, p.title, p.cookUserId, p.notes, p.reminderMinutes, a.user.id).run();
    await recordHouseholdActivity(c, a.householdId, a.user.id, "meal.planned", `${actor} asked Silvi to plan ${p.title} for ${p.mealType}.`);
    return `Planned ${p.title} for ${p.mealType} on ${p.mealDate}.`;
  }
  if (action.type === "routine.create") {
    const id = crypto.randomUUID();
    await c.env.DB.prepare("INSERT INTO household_routines(id,household_id,title,notes,cadence,assignee_user_id,next_due_at,reminder_minutes,created_by) VALUES(?,?,?,?,?,?,?,?,?)")
      .bind(id, a.householdId, p.title, p.notes, p.cadence, p.assigneeUserId, p.nextDueAt, p.reminderMinutes, a.user.id).run();
    await recordHouseholdActivity(c, a.householdId, a.user.id, "routine.add", `${actor} asked Silvi to add routine “${p.title}”.`);
    return `Created routine “${p.title}”.`;
  }
  if (action.type === "routine.complete") {
    const routine = await c.env.DB.prepare("SELECT title,cadence,next_due_at nextDueAt,assignee_user_id assigneeUserId FROM household_routines WHERE id=? AND household_id=? AND active=1").bind(p.routineId, a.householdId).first<{ title: string; cadence: string; nextDueAt: string | null; assigneeUserId: string | null }>();
    if (!routine) throw new Error("That routine is no longer active.");
    if (routine.assigneeUserId && routine.assigneeUserId !== a.user.id && !routineManager(a.role)) throw new Error("That routine is assigned to another household member.");
    await c.env.DB.batch([
      c.env.DB.prepare("INSERT INTO household_routine_completions(id,routine_id,household_id,completed_by) VALUES(?,?,?,?)").bind(crypto.randomUUID(), p.routineId, a.householdId, a.user.id),
      c.env.DB.prepare("UPDATE household_routines SET next_due_at=?,snoozed_until=NULL,last_notified_due_at=NULL,updated_at=datetime('now') WHERE id=? AND household_id=?").bind(nextDue(routine.nextDueAt, routine.cadence), p.routineId, a.householdId),
    ]);
    await recordHouseholdActivity(c, a.householdId, a.user.id, "routine.completed", `${actor} completed “${routine.title}” with Silvi.`);
    return `Marked routine “${routine.title}” as done.`;
  }
...
app.get("/api/v1/households/:householdId/silvi/context", async c => {
  ...
  return c.json(await context(c, a.householdId, a.user.id, a.role));
});
...
app.post("/api/v1/households/:householdId/silvi/ask", async c => {
  ...
  const householdContext = await context(c, a.householdId, a.user.id, a.role);
  const system = `You are Silvi, the private household assistant inside Kit Hub. Answer only from the supplied household context. Be warm, concise and practical. Never invent household facts. You may PROPOSE one household action when the user clearly asks to change something, but you must never claim the action already happened. Supported actions are task.create, task.complete, event.create, meal.plan, routine.create, routine.complete. Only propose actions allowed by capabilities. Use member IDs and task/routine IDs exactly from context. Resolve relative dates using currentTime and the user's timeZone when available. If required details are missing or ambiguous, ask a short clarification question and set action to null. Return ONLY JSON in this exact shape: {"answer":"plain-language response","action":null} or {"answer":"plain-language response explaining the proposed change and that confirmation is required","action":{"type":"supported.type","payload":{}}}. For event.create, startsAt/endsAt must be ISO timestamps. For meal.plan, mealDate must be YYYY-MM-DD. Do not include markdown fences.`;
  const prompt = `Current user: ${a.user.name ?? "Household member"}\nHousehold context JSON:\n${JSON.stringify(householdContext)}\n\nUser request: ${question}`;
  try {
    const result: any = await c.env.AI.run("@cf/meta/llama-3.1-8b-instruct-fast", { messages: [{ role: "system", content: system }, { role: "user", content: prompt }], max_tokens: 700, temperature: .15 });
    const raw = clean(result?.response ?? result?.result?.response ?? result?.text, 6000);
    if (!raw) return apiError(c, 500, "SILVI_EMPTY_RESPONSE", "Silvi could not form an answer yet.");
    const envelope = parseEnvelope(raw);
    const action = await normalizeAction(c, a, envelope.action);
    if (!action) return c.json({ answer: envelope.answer, generatedAt: isoNow(), requiresConfirmation: false });
    const id = crypto.randomUUID();
    const expiresAt = new Date(Date.now() + 10 * 60 * 1000).toISOString();
    await c.env.DB.prepare("INSERT INTO silvi_action_proposals(id,household_id,user_id,action_type,summary,payload_json,expires_at) VALUES(?,?,?,?,?,?,?)")
      .bind(id, a.householdId, a.user.id, action.type, action.summary, JSON.stringify(action.payload), expiresAt).run();
...
app.post("/api/v1/households/:householdId/silvi/actions/:proposalId/confirm", async c => {
  ...
  if (body?.confirm !== true) return apiError(c, 422, "CONFIRMATION_REQUIRED", "Confirm this Silvi action before applying it.");
  const row = await c.env.DB.prepare("SELECT id,action_type actionType,summary,payload_json payloadJson,status,expires_at expiresAt FROM silvi_action_proposals WHERE id=? AND household_id=? AND user_id=?")
    .bind(c.req.param("proposalId"), a.householdId, a.user.id).first<{ id: string; actionType: ActionType; summary: string; payloadJson: string; status: string; expiresAt: string }>();
  if (!row) return apiError(c, 404, "SILVI_PROPOSAL_NOT_FOUND", "That Silvi proposal could not be found.");
  ...
  if (Date.parse(row.expiresAt) <= Date.now()) {
    await c.env.DB.prepare("UPDATE silvi_action_proposals SET status='cancelled' WHERE id=? AND status='pending'").bind(row.id).run();
    return apiError(c, 409, "SILVI_PROPOSAL_EXPIRED", "That Silvi proposal expired. Ask Silvi again so it can check the latest household information.");
  }
  const claimed = await c.env.DB.prepare("UPDATE silvi_action_proposals SET status='executing' WHERE id=? AND household_id=? AND user_id=? AND status='pending'")
    .bind(row.id, a.householdId, a.user.id).run();
  if (!claimed.meta.changes) return apiError(c, 409, "SILVI_PROPOSAL_USED", "That Silvi proposal is already being handled.");
  try {
    const rawPayload = JSON.parse(row.payloadJson) as Record<string, unknown>;
    const action = await normalizeAction(c, a, { type: row.actionType, payload: rawPayload });
    if (!action) throw new Error("The household changed and this proposal is no longer valid.");
    const result = await executeAction(c, a, action);
    ...
    return c.json({ ok: true, result, proposalId: row.id });
  } catch (error) {
    await c.env.DB.prepare("UPDATE silvi_action_proposals SET status='failed' WHERE id=?").bind(row.id).run();
    return apiError(c, 422, "SILVI_ACTION_FAILED", error instanceof Error ? error.message : "Silvi could not apply that change. Nothing else was changed.");
  }
});

app.post("/api/v1/households/:householdId/silvi/actions/:proposalId/cancel", async c => {
  const a = await access(c); if ("response" in a) return a.response;
  const result = await c.env.DB.prepare("UPDATE silvi_action_proposals SET status='cancelled' WHERE id=? AND household_id=? AND user_id=? AND status='pending'")
    .bind(c.req.param("proposalId"), a.householdId, a.user.id).run();
  if (!result.meta.changes) return apiError(c, 409, "SILVI_PROPOSAL_NOT_PENDING", "That Silvi proposal is no longer waiting for a decision.");
