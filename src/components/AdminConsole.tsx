...
function PullRequestSummary({ pull }: { pull: AdminPublishPullRequest }) {
  return <div className={`admin-publish-summary ${pull.ready ? "ready" : "blocked"}`}>
    ...
    <span>{pull.ready ? <CheckCircle2 /> : <TriangleAlert />}{pull.ready ? pull.draft ? "Ready after approval" : "Ready" : "Blocked"}</span>
    {pull.draft && pull.ready && <small className="admin-publish-draft-note">Draft · Kit Hub will mark it ready after approval.</small>}
...
export function AdminConsole({ onBack }: { onBack: () => void }) {
  ...
  const format = (value?: string | null) => value
    ...
    : "Not available";
  const stages = [
    { label: "Publish", hint: selectedPull ? `${selectedPull.draft ? "Ready + merge" : "Merge"} PR #${selectedPull.number}` : "Use current main", stepNames: [] as string[] },
  ...
  return <main className={`admin-page admin-theme-${theme}`}>
    ...
    <div className="admin-grid">
      <section className="admin-card">
        ...
        <p>{hasOpenPullRequests ? "Choose a checked pull request. After confirmation, Kit Hub marks drafts ready, squash-merges the exact checked commit to main, then runs and verifies the protected production workflow." : "No open pull request targets main. This will release and verify the current main branch."}</p>
        ...
        <button className="admin-primary" onClick={() => setConfirmOpen(true)} disabled={!canStart}>{triggering || releaseActive ? <LoaderCircle className="spin" /> : <Rocket />}{triggering ? "Starting sequence…" : releaseActive ? "Release in progress…" : selectedPull ? `${selectedPull.draft ? "Approve + " : ""}publish PR #${selectedPull.number} + release` : "Release current main + verify"}</button>
      </section>
    ...
    {confirmOpen && <div className="admin-modal-backdrop"><section className="admin-completion confirm" role="dialog" aria-modal="true" aria-labelledby="admin-release-confirm-title"><button className="admin-modal-close" onClick={() => setConfirmOpen(false)} aria-label="Close"><X /></button><Rocket /><small>SYNC + RELEASE + VERIFY</small><h2 id="admin-release-confirm-title">{selectedPull ? `${selectedPull.draft ? "Approve and publish" : "Publish"} PR #${selectedPull.number}?` : "Release current main?"}</h2>{selectedPull ? <><p><strong>{selectedPull.title}</strong></p><p>{selectedPull.draft && <>This first marks the draft ready for review, which can notify configured reviewers. </>}Then Kit Hub squash-merges commit <code>{selectedPull.headSha.slice(0, 7)}</code> into <code>main</code> and starts the protected production workflow.</p></> : <p>This starts the protected workflow for the current <code>main</code> branch. No pull request will be merged.</p>}<p className="admin-confirm-impact">Production may apply D1 migrations and deploy the Worker. GitHub will still require its configured production approval.</p><div className="admin-modal-actions"><button className="admin-secondary" onClick={() => setConfirmOpen(false)}>Cancel</button><button className="admin-primary" onClick={() => void triggerRelease()}>{selectedPull ? selectedPull.draft ? "Approve, publish and release" : "Publish and release" : "Release and verify"}</button></div></section></div>}
