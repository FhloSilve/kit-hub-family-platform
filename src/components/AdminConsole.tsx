...
const themes: Array<{ key: AdminTheme; label: string; hex: string }> = [
  { key: "orchid", label: "Orchid", hex: "#F2CFF1" }, { key: "apricot", label: "Apricot", hex: "#EAB099" },
  { key: "periwinkle", label: "Periwinkle", hex: "#AFAFDA" }, { key: "ocean", label: "Ocean", hex: "#19485F" },
];
...
export function AdminConsole({ onBack }: { onBack: () => void }) {
  ...
  const [loading, setLoading] = useState(true); const [triggering, setTriggering] = useState(false); const [stopping, setStopping] = useState(false);
  ...
  const [completion, setCompletion] = useState<"success" | "failure" | "cancelled" | null>(null); const [waitingForRun, setWaitingForRun] = useState(false);
  ...
  async function triggerRelease() { setTriggering(true); setMessage(null); setError(null); setCompletion(null); releaseStart.current = { runId: status?.latestRun?.id ?? null, requestedAt: Date.now() }; setWaitingForRun(true); try { const result = await api.triggerAdminRelease(); setMessage(result.message); window.setTimeout(() => void refresh(), 1200); } catch (e) { releaseStart.current = null; setWaitingForRun(false); setError(e instanceof ApiError ? e.message : "The release could not be started."); setCompletion("failure"); } finally { setTriggering(false); } }
  async function stopRelease() { if (!trackedRun || isFinished(trackedRun)) return; setStopping(true); setError(null); try { const result = await api.cancelAdminRelease(trackedRun.id); setMessage(result.message); window.setTimeout(() => void refresh(), 800); } catch (e) { setStopping(false); setError(e instanceof ApiError ? e.message : "The release could not be stopped."); setCompletion("failure"); } }
  ...
  return <main className={`admin-page admin-theme-${theme}`}>
    ...
    {releaseActive && <section className="admin-progress"><header><div><LoaderCircle className="spin" /><span><strong>{stopping ? "Stopping production release…" : trackedRun ? "Production release is running" : "Waiting for GitHub to start the run"}</strong><small>{stopping ? "GitHub is finishing cancellation of the active job." : "This page updates automatically. You can keep it open."}</small></span></div><div className="admin-progress-actions"><b>{trackedRun ? "LIVE" : "STARTING"}</b>{trackedRun && !isFinished(trackedRun) && <button className="admin-stop" type="button" onClick={() => void stopRelease()} disabled={stopping}><Square />{stopping ? "Stopping…" : "Stop release"}</button>}</div></header><div className="admin-stage-list">{stages.map((stage, index) => { const state = stageState(index); return <div className={`admin-stage ${state}`} key={stage.label}><span>{state === "done" ? <Check /> : state === "failed" ? <X /> : state === "active" ? <LoaderCircle className="spin" /> : index + 1}</span><div><strong>{stage.label}</strong><small>{stage.hint}</small></div></div>; })}</div></section>}
    {error && <div className="admin-alert error"><TriangleAlert /> <span><strong>Couldn&apos;t complete that action</strong><small>{error}</small></span></div>}{message && releaseActive && <div className="admin-alert success"><CheckCircle2 /> <span><strong>{stopping ? "Cancellation requested" : "Release requested"}</strong><small>{message}</small></span></div>}
    <div className="admin-grid"><section className="admin-card"><header><Rocket /><div><small>PRIMARY ACTION</small><h2>Sync + release + verify</h2></div></header><p>Runs the protected production workflow and follows it until the final result.</p><button className="admin-primary" onClick={() => void triggerRelease()} disabled={!ready || triggering || releaseActive}>{triggering || releaseActive ? <LoaderCircle className="spin" /> : <Rocket />}{triggering ? "Starting release…" : releaseActive ? "Release in progress…" : "Sync + release + verify"}</button></section><section className="admin-card"><header><CloudCog /><div><small>LIVE WORKER</small><h2>Current deployment</h2></div></header>{loading ? <p>Loading deployment metadata…</p> : <dl><div><dt>Version</dt><dd>{status?.deployedVersion.id ?? "Unknown"}</dd></div><div><dt>Tag</dt><dd>{status?.deployedVersion.tag ?? "No tag"}</dd></div><div><dt>Deployed</dt><dd>{format(status?.deployedVersion.timestamp)}</dd></div></dl>}</section><section className="admin-card wide"><header><GitBranch /><div><small>GITHUB ACTIONS</small><h2>Latest production release</h2></div><button className="admin-secondary" onClick={() => void refresh()}><RefreshCw /> Refresh</button></header>{run ? <div className="admin-run"><strong>{run.status !== "completed" ? "Release in progress" : run.conclusion === "success" ? "Release succeeded" : run.conclusion === "cancelled" ? "Release cancelled" : "Release needs attention"}</strong><small>{run.name} · {run.headBranch} · {format(run.updatedAt)}</small>{run.htmlUrl && <a href={run.htmlUrl} target="_blank" rel="noreferrer">Open run <ExternalLink /></a>}</div> : <p>No production release workflow run is available yet.</p>}</section></div>
