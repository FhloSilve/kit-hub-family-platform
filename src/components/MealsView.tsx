...
export function MealsView({ data, members, loading, householdId, demo, onChange, onGroceriesAdded }: Props) {
  ...
  const [ingredientRecipe, setIngredientRecipe] = useState<MealRecipe | null>(null);
  ...
  function groceriesAdded(items: GroceryItem[]) {
    onGroceriesAdded(items);
    setMessage(`${items.length} ingredient${items.length === 1 ? "" : "s"} added to Groceries.`);
  }
  ...
  return <>
    <header className="today-heading module-heading meals-heading">
      <div><span className="today-date">Milestone 5</span><h1>Meals &amp; Dinner</h1><p>Plan the week, keep favourite recipes close, and send exactly what you need straight to Groceries.</p></div>
    ...
    <section className="meal-sharing-card" aria-label="Meal sharing and permissions"><span><UsersRound /></span><div><small>Shared household planner</small><strong>{data.canManage ? "You can add, edit, and remove meals." : "You can view meals, suggest ideas, and vote."}</strong><p>Everyone in this household can see the meal plan. Owners, admins, adults, and teens can manage it by default; individual permissions can change that.</p></div></section>

    <section className="meal-dietary-card"><span><Heart /></span><div><small>Dietary notes &amp; allergies</small><strong>{data.dietaryNotes || "Nothing recorded yet"}</strong></div>{data.canManage && <button onClick={() => setEditingDietary(true)}><Pencil /> Edit</button>}</section>

    <section className="meal-week-card">
      <header className="meal-week-toolbar"><div><CalendarDays /><span><strong>{weekStart.toLocaleDateString(undefined, { month: "long", day: "numeric" })} – {weekEnd.toLocaleDateString(undefined, { month: "long", day: "numeric" })}</strong><small>Your household week</small></span></div><nav><button aria-label="Previous week" onClick={() => setWeekStart(addDays(weekStart, -7))}><ChevronLeft /></button><button onClick={() => setWeekStart(startOfWeek(new Date()))}>Today</button><button aria-label="Next week" onClick={() => setWeekStart(addDays(weekStart, 7))}><ChevronRight /></button></nav></header>
      {loading ? <div className="meal-loading">Opening the family menu…</div> : <div className="meal-week-grid">{days.map((day) => {
        const key = dateKey(day);
        const today = key === dateKey(new Date());
        return <article key={key} className={today ? "is-today" : ""}><header><small>{day.toLocaleDateString(undefined, { weekday: "short" })}</small><strong>{day.getDate()}</strong></header><div>{mealTypes.map((meal) => {
          const plan = data.plans.find((item) => item.mealDate === key && item.mealType === meal.value) ?? null;
          return <button key={meal.value} className={plan ? "meal-slot is-planned" : "meal-slot"} disabled={!data.canManage && !plan} onClick={() => data.canManage && setMealEditor({ date: key, type: meal.value, plan })}><span>{meal.emoji}</span><small>{meal.label}</small>{plan ? <><strong>{plan.title}</strong>{plan.cookName && <em><ChefHat /> {plan.cookName}</em>}</> : <i><Plus /> Add</i>}</button>;
        })}</div></article>;
      })}</div>}
    </section>
    ...
    <div className="meal-lower-grid">
      <section className="meal-library-card">
        ...
        <div className="meal-recipe-list">{data.recipes.map((recipe) => <article key={recipe.id}>
          ...
          <footer><button disabled={!recipe.ingredients.length} onClick={() => setIngredientRecipe(recipe)}><ShoppingBasket /> Choose ingredients</button>{data.canManage && <button onClick={() => setRecipeEditor(recipe)}><Pencil /> Edit</button>}</footer>
        </article>)}{!data.recipes.length && <button className="meal-empty" disabled={!data.canManage} onClick={() => setRecipeEditor("new")}><CookingPot /><strong>Your family cookbook is empty.</strong><small>Add a favourite meal and its ingredients.</small></button>}</div>
      </section>

      <section className="meal-suggestions-card"><header><div><Lightbulb /><span><small>Family choice</small><h2>Meal ideas</h2></span></div><button onClick={() => setSuggesting(true)}><Plus /> Suggest</button></header><div className="meal-suggestion-list">{data.suggestions.map((suggestion) => <article key={suggestion.id}><div><span>{mealTypes.find((meal) => meal.value === suggestion.mealType)?.emoji}</span><p><strong>{suggestion.title}</strong><small>{suggestion.suggestedByName}{suggestion.notes ? ` · ${suggestion.notes}` : ""}</small></p></div><button className={suggestion.votedByMe ? "is-voted" : ""} onClick={() => void toggleVote(suggestion)}><ThumbsUp fill={suggestion.votedByMe ? "currentColor" : "none"} /> {suggestion.votes}</button></article>)}{!data.suggestions.length && <div className="meal-suggestions-empty"><Lightbulb /><strong>No meal ideas yet.</strong><small>Everyone in the household can suggest and vote.</small></div>}</div></section>
    </div>

    {mealEditor && <MealPlanModal editor={mealEditor} recipes={data.recipes} members={members} householdId={householdId} demo={demo} onClose={() => setMealEditor(null)} onSaved={updatePlan} onDelete={removePlan} onChooseIngredients={setIngredientRecipe} />}
    ...
    {ingredientRecipe && <MealIngredientPicker recipe={ingredientRecipe} householdId={householdId} demo={demo} onClose={() => setIngredientRecipe(null)} onAdded={groceriesAdded} />}
...
function MealPlanModal({ editor, recipes, members, householdId, demo, onClose, onSaved, onDelete, onChooseIngredients }: {
  editor: NonNullable<MealEditor>;
  recipes: MealRecipe[];
  members: HouseholdMemberSummary[];
  householdId: string;
  demo: boolean;
  ...
  onDelete: (plan: MealPlan) => Promise<void>;
  onChooseIngredients: (recipe: MealRecipe) => void;
}) {
  ...
  return <MealModal title={editor.plan ? "Edit planned meal" : "Plan a meal"} eyebrow="Weekly menu" onClose={onClose}><form onSubmit={submit}>
    ...
    {selectedRecipe && <section className="meal-plan-grocery-box"><div><ShoppingBasket /><span><strong>Need ingredients?</strong><small>{selectedRecipe.ingredients.length} item{selectedRecipe.ingredients.length === 1 ? "" : "s"} in this recipe</small></span></div><button type="button" disabled={!selectedRecipe.ingredients.length} onClick={() => onChooseIngredients(selectedRecipe)}>Choose items for Groceries</button></section>}
...
function SuggestionModal({ householdId, demo, onClose, onSaved }: { householdId: string; demo: boolean; onClose: () => void; onSaved: (suggestion: MealSuggestion) => void }) {
  ...
  async function submit(event: FormEvent<HTMLFormElement>) { event.preventDefault(); setBusy(true); setError(null); const form = new FormData(event.currentTarget); const input = { title: String(form.get("title") || ""), notes: String(form.get("notes") || ""), mealType: String(form.get("mealType") || "dinner") as MealType }; try { const saved = demo ? { id: crypto.randomUUID(), ...input, notes: input.notes || null, suggestedByUserId: "demo-user", suggestedByName: "Demo", votes: 1, votedByMe: true, createdAt: new Date().toISOString() } : await api.createMealSuggestion(householdId, input); onSaved(saved); } catch (caught) { setError(caught instanceof Error ? caught.message : "The meal idea could not be shared."); } finally { setBusy(false); } }
