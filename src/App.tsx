...
function ConnectedApp() {
        ...
        if (!cancelled) {
          setBootstrapError(
            error instanceof ApiError
              ...
              : { message: "Kit Hub could not load your household." },
          );
        }
      })
      .finally(() => {
  ...
  function handleHouseholdCreated(household: HouseholdSummary) {
    if (!bootstrap) return;
    setBootstrap({
      ...bootstrap,
      households: [...bootstrap.households, household],
  ...
  if (bootstrapError && !bootstrap) {
    return (
      <main className="fatal-state">
        <Brand />
        <div>
          <h1>We couldn&apos;t open Kit Hub.</h1>
          <p>{bootstrapError.message}</p>
          {bootstrapError.requestId && <small className="request-reference">Reference: {bootstrapError.requestId}</small>}
          <div className="fatal-state__actions">
            <button className="button button--primary" type="button" onClick={() => setBootstrapAttempt((attempt) => attempt + 1)}>Try again</button>
  ...
  if (!bootstrap.activeHousehold) {
    return <HouseholdOnboarding bootstrap={bootstrap} onCreated={handleHouseholdCreated} onSignOut={signOut} />;
  }
...
function LoadingScreen({ label }: { label: string }) {
  return <main className="loading-screen"><Brand /><span className="loading-orbit" aria-hidden="true"><i /></span><p>{label}</p></main>;
}
