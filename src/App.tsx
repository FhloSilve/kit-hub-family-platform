...
function ConnectedApp() {
  ...
  useEffect(() => {
    ...
    void api
      ...
      .catch((error: unknown) => {
        if (!cancelled) {
          setBootstrapError(error instanceof ApiError ? error.message : "Kit Hub could not load your household.");
        }
      })
      .finally(() => {
  ...
  if (bootstrapError && !bootstrap) {
    return (
      <main className="fatal-state">
        <Brand />
        <div><h1>We couldn&apos;t open Kit Hub.</h1><p>{bootstrapError}</p><button className="button button--primary" type="button" onClick={() => window.location.reload()}>Try again</button></div>
      </main>
    );
  }
