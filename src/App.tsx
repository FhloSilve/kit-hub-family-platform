...
export default function App(){if(isAdminPath)return <AdminRoute/>;if(isDemo)return <><PullToRefresh/><TodayDashboard bootstrap={demoBootstrap} demo onSignOut={async()=>{window.history.replaceState({},"",window.location.pathname);window.location.reload()}}/></>;return <><PullToRefresh/><ConnectedApp/><AppUpdatePrompt/></>}
